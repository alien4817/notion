...
若 DSMS 同時對 HTTP/HTTPS 都回傳 `socket hang up` 或 `ERR_EMPTY_RESPONSE`，monitor 會視為上游網站暫時無法從該 runner 存取，保留既有 snapshot、略過 Notion 寫入，並以成功狀態結束，避免排程因外部網站阻擋雲端 IP 而持續失敗。
